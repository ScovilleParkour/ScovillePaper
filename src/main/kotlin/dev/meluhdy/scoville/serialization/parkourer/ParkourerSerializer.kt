...
object ParkourerSerializer: MelodiaSerializer<Parkourer>() {
    ...
    class ParkourerBuilder: Builder<Parkourer>() {
        ...
        val achievements: MutableList<UUID> = mutableListOf()
    ...
    override val steps: Array<SerializerElement<*, Parkourer>> = arrayOf(
        ...
        SerializerElement<List<String>, Parkourer>("achievements", ListSerializer(String.serializer()), { it.getAchievementList().map { achievement -> achievement.toString() } }, { list, builder -> (builder as ParkourerBuilder).achievements.addAll(list.map { uuid -> UUID.fromString(uuid) }) })
    )
